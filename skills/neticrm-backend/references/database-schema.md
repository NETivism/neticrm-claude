...
## 9. After Modifying Schema

After modifying any XML schema file, two more steps are required:

### Step 1: Run GenCode to regenerate DAO classes

**Before running, ask the developer:**
> "Does your environment have a specific GenCode command? (e.g. running inside Docker or another container)"

- If yes → use the command the developer provides
- If no → fall back to the generic command:
...
Important notes:
- Never manually edit DAO files under `/CRM/**/DAO/` — they are fully auto-generated by GenCode
- Always regenerate after any XML schema change to keep DAO in sync
- Commit the XML schema and the generated DAO file together

### Step 2: Write a migration script

When adding fields or tables, create an update script at `/neticrm/neticrm_update/update/neticrm_xNNN.inc`.
For migration script templates and idempotent guard patterns, see the Workflow section in `.claude/agents/database-agent.md`.
